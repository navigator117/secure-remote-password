...
exports.deriveSession = function (serverSecretEphemeral, clientPublicEphemeral, salt, username, verifier, clientSessionProof, callback) {
    // N    A large safe prime (N = 2q+1, where q is prime)
    // g    A generator modulo N
    // k    Multiplier parameter (k = H(N, g) in SRP-6a, k = 3 for legacy SRP-6)
    ...
    if (A.mod(N).equals(SRPInteger.ZERO)) {      
        callback(new Error('The client sent an invalid public ephemeral'))
        return
    }
    ...
    if (!actual.equals(expected)) {
        callback(new Error('Client provided session proof is invalid'))
        return
    }
